...
import { fetchInitialAlerts, selectAlert, closeDetail } from '../store/alertsSlice';
...
export default function Dashboard() {
  ...
  const { items: alerts, selectedAlertId, status } = useSelector((state) => state.alerts);
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden bg-[var(--bg-color)]">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className={`flex flex-col ${selectedAlertId ? 'w-full lg:w-[calc(100%-400px)] lg:border-r border-[var(--panel-border)]' : 'w-full'} overflow-hidden transition-all duration-300`}>
           <AlertList 
            alerts={alerts} 
        ...
        {selectedAlertId && (
          <div className="w-[400px] flex-none bg-[var(--panel-bg)] overflow-y-auto hidden lg:block border-l border-[var(--panel-border)] animate-slideIn">
            <AlertDetail alert={selectedAlert} onClose={() => dispatch(closeDetail())} />
          </div>
        )}
      </div>
    </div>
