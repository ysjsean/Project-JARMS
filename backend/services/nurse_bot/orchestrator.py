...
class NurseBotOrchestrator:
    ...
    async def connect(self):
        """
        Connects to the OpenAI Realtime API and sets up the session.
        """
        headers = {
        ...
        try:
            self.openai_ws = await websockets.connect(OPENAI_REALTIME_URL, extra_headers=headers)
            print(f"[NURSE_BOT] Connected to OpenAI Realtime for Case {self.case_id}")
            
            # Initialize session with JARMS persona and tools
            ...
            # In Realtime API, we can trigger a response by sending a conversation item
            # or just letting the VAD (Voice Activity Detection) handle it.
            # We'll start by providing the 'Conversation Starter' textually.
    ...
    async def _handle_openai_message(self, message_str: str):
        ...
        if msg_type == "response.function_call_arguments.done":
            ...
            if fn_name == "get_case_details":
                ...
                result = await tools.escalate_urgency(self.case_id, args.get("new_bucket"), args.get("reason"))
        ...
        if msg_type in ["response.audio.delta", "response.audio_transcript.delta", "error"]:
