...
async def get_case_details(case_id: str) -> Dict[str, Any]:
    ...
    """
    try:
        res = supabase.table("cases").select("*").eq("case_id", case_id).single().execute()
        if not res.data:
        ...
        return {
            ...
            "ai_reasoning": case.get("sbar_json", {}).get("assessment", "No reasoning available")
        }
...
async def get_beneficiary_history(nric: str) -> Dict[str, Any]:
    ...
    try:
        ...
        res = supabase.table("pab_beneficiaries").select("*").eq("nric", nric).single().execute()
        if not res.data:
...
async def escalate_urgency(case_id: str, new_bucket: str, reason: str) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        update_payload = {
            ...
            "status": "assigned" if new_bucket in ["life_threatening", "emergency"] else "processing",
        }
        
        # We append the nurse bot reason to the existing assessment if possible
        res_current = supabase.table("cases").select("sbar_json").eq("case_id", case_id).single().execute()
        sbar = res_current.data.get("sbar_json") or {}
        
        if isinstance(sbar, dict):
            sbar["assessment"] = f"{sbar.get('assessment', '')}\n\n[Nurse Bot Escalation]: {reason}"
...
async def record_nurse_note(case_id: str, note: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if isinstance(sbar, dict):
            ...
            supabase.table("cases").update({"sbar_json": sbar}).eq("case_id", case_id).execute()
